...
let mytoken= ['dyq'];//快速订阅访问入口, 留空则不启动快速订阅
...
let addressesnotls = [
	//'www.visa.com.sg#从这往下',
	//'www.wto.org:8080#都是notls',
	//'www.who.int:8880#慎用！！！',
];
...
let subconverter = "subapi-loadbalancing.pages.dev"; //在线订阅转换后端。支持自建psub 可自行搭建https://github.com/bulianglin/psub
let subconfig = "https://raw.githubusercontent.com/LQwbd/ACL4SSR/main/Clash/config/ACL4SSR_Online_Full_MultiMode.ini"; //订阅转换配置文件
...
async function getAddressesapi(api) {
	...
	try {
		...
		const responses = await Promise.allSettled(api.map(apiUrl => fetch(apiUrl, {
			...
			headers: {
				'Accept': 'text/html,application/xhtml+xml,application/xml;',
				'User-Agent': 'LQwbd/WorkerVless2sub'
			},
		...
		if (!userAgent.includes('subconverter') && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
			...
			try {
				...
				return new Response(subconverterContent, {
					headers: { 
						...
						"Subscription-Userinfo": `upload=0; download=${UD}; total=${total}; expire=${expire}`,
					},
				});
			} catch (error) {
			...
			if(noTLS == true){
				...
				notlsresponseBody = uniqueAddressesnotls.map(address => {
					...
					if (edgetunnel.trim() === 'LQwbd' && RproxyIP.trim() === 'true') {
					// 将addressid转换为小写
